...
import { Edit, MapPin, Clock, Phone, Heart, Star, Users, Check, X, Plus } from "lucide-react";
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-white">
                <h1 className="text-xl md:text-2xl font-bold mb-1 text-shadow-lg">{establecimiento.nombre}</h1>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${
              establecimiento.estado === "activo" ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : 
              establecimiento.estado === "pendiente" ? "bg-amber-100 text-amber-700 border border-amber-200" : 
              "bg-red-100 text-red-700 border border-red-200"
            }`}>
              {establecimiento.estado === "activo" ? <Check size={12} /> : 
               establecimiento.estado === "pendiente" ? <Clock size={12} /> : 
               <X size={12} />}
          ...
          <div className="flex items-center gap-6 text-sm">
            <div className="flex items-center gap-1.5 text-slate-600">
              ...
              <span className="font-medium">{formatNumber(likes[establecimiento._id] || 0)}</span>
            </div>
            <div className="flex items-center gap-1.5 text-slate-600">
              <div className="p-1.5 bg-blue-100 rounded-lg">
                <Users size={14} className="text-blue-500" />
              </div>
              <span className="font-medium">{formatNumber(seguidores[establecimiento._id] || 0)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-slate-800">Descripción</h3>
              ...
              {editing ? (
                ...
                <p className="text-slate-600 leading-relaxed">
                  {!establecimiento.descripcion ? (
                    <span className="text-slate-400 italic">Sin descripción disponible</span>
                  ) : (
                    showFullDescription 
                      ? establecimiento.descripcion 
                      : `${establecimiento.descripcion.substring(0, 150)}${establecimiento.descripcion.length > 150 ? '...' : ''}`
                  )}
                </p>
              )}
            </div>
            ...
            {/* Categorías y tipos */}
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Categorías y tipos</h3>
              {editing ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Categorías</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Tipos</label>
                    <div className="flex flex-wrap gap-2">
                      {establecimiento.tipo && establecimiento.tipo.map((tip, i) => (
                        <div key={i} className="flex items-center bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-xl transition-colors border border-slate-200">
                          <span className="text-sm font-medium text-slate-700">{tip.tipo_nombre}</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {establecimiento.categoria && establecimiento.categoria.map((cat, i) => (
                    <span key={i} className="px-4 py-2 text-sm font-medium rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-200">
                      {cat.nombre}
                    </span>
                  ))}
                  {establecimiento.tipo && establecimiento.tipo.map((tip, i) => (
                    <span key={i} className="px-4 py-2 bg-blue-50 text-blue-600 text-sm font-medium rounded-xl border border-blue-200">
                      {tip.tipo_nombre}
                    </span>
                  ))}
                  {(!establecimiento.categoria || establecimiento.categoria.length === 0) && 
                   (!establecimiento.tipo || establecimiento.tipo.length === 0) && (
                    <span className="text-slate-400 italic">No hay categorías definidas</span>
                  )}
                </div>
              )}
            </div>
            ...
            {/* Horarios */}
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Horarios de atención</h3>
              {editing ? (
                <div className="space-y-3">
                  {horarios.map((hor, i) => (
                    <div key={i} className="flex flex-col sm:flex-row gap-3 p-4 bg-white rounded-xl border border-slate-200">
                      <select
                      ...
                      />
                      <span className="flex items-center justify-center text-slate-400 font-medium">a</span>
                      <input
                        className="flex-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                        type="time"
                        value={hor.salida}
                  ...
                  {establecimiento.horario && establecimiento.horario.length > 0 ? (
                    establecimiento.horario.map((hor, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100">
                        ...
                        <span className="font-semibold text-slate-700 min-w-20">{hor.dia}:</span>
                        <span className="text-slate-600">{hor.entrada} - {hor.salida}</span>
                      </div>
                    ))
                  ) : (
                    <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl">
                      <Clock size={24} className="mx-auto text-slate-400 mb-2" />
                      <p className="text-slate-400">No hay horarios definidos</p>
                    </div>
                  )}
                </div>
              )}
            ...
            {/* Teléfono */}
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Contacto</h3>
              {editing ? (
          ...
          <div className="space-y-6">
            {/* Ubicación */}
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Ubicación</h3>
              {editing ? (
                <div className="space-y-4">
                  <input
                    ref={direccionRef}
                    ...
                    {establecimiento.ubicacion && establecimiento.ubicacion.length > 0 ? (
                      <div className="flex items-start gap-3 p-4 bg-white rounded-xl border border-slate-100">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-slate-800 mb-1">{establecimiento.ubicacion[0].direccion}</p>
                          <p className="text-slate-600">{establecimiento.ubicacion[0].distrito}, {establecimiento.ubicacion[0].ciudad}</p>
                          {establecimiento.ubicacion[0].codigoPostal && (
                            <p className="text-slate-500 text-sm">CP: {establecimiento.ubicacion[0].codigoPostal}</p>
                          )}
                          {establecimiento.ubicacion[0].referencia && (
                            <p className="text-slate-500 text-sm mt-2 italic">{establecimiento.ubicacion[0].referencia}</p>
                          )}
                        </div>
                      ...
                      <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl">
                        ...
                        <p className="text-slate-400">No hay información de ubicación disponible</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-slate-100 to-slate-200 rounded-2xl h-48 flex items-center justify-center relative overflow-hidden border border-slate-200">
                    ...
                    <div className="text-slate-500 text-center z-10">
                      <div className="p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-white/20 shadow-lg">
                        ...
                        <p className="text-sm text-slate-400 mt-1">Integración pendiente</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-gradient-to-br from-slate-50 to-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Comentarios y reseñas</h3>
              <div className="max-h-96 overflow-y-auto">
                {establecimiento.comentarios && establecimiento.comentarios.length > 0 ? (
                  <div className="space-y-4">
                    {establecimiento.comentarios.map((comment, i) => (
                      <div key={i} className="p-4 bg-white rounded-xl border border-slate-100 hover:shadow-md transition-shadow">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">
                            {comment.usuario?.nombreUsuario?.charAt(0)?.toUpperCase() || "?"}
                          </div>
                          <div className="flex-1">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                              <p className="font-semibold text-slate-800">
                                {comment.usuario?.nombreUsuario || "Usuario anónimo"}
                              </p>
                        ...
                        <p className="text-slate-600 leading-relaxed ml-13">{comment.comentario}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl">
                    <div className="p-4 bg-white/50 backdrop-blur-sm rounded-xl border border-white/20 shadow-sm inline-block">
                      ...
                      <p className="text-slate-400 text-sm mt-1">Sé el primero en dejar una reseña</p>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-200">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div className="p-3 bg-amber-50 rounded-xl border border-amber-200">
                      ...
                      <div className="text-xs text-amber-600 font-medium">Promedio</div>
                    </div>
                    <div className="p-3 bg-blue-50 rounded-xl border border-blue-200">
                      <div className="text-lg font-bold text-blue-700">
                        {establecimiento.comentarios.length}
                      </div>
                      <div className="text-xs text-blue-600 font-medium">Reseñas</div>
                    </div>
                    <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                      <div className="text-lg font-bold text-emerald-700">
                        {Math.round((establecimiento.comentarios.filter(c => c.calificacion >= 4).length / establecimiento.comentarios.length) * 100)}%
                      </div>
                      <div className="text-xs text-emerald-600 font-medium">Positivas</div>
                    </div>
                  </div>
                </div>
              )}
